...
export default function AdminProducts() {
  ...
  const save = async (e) => {
    ...
    if (form.onSale && (!salePrice || salePrice >= price)) {
    ...
    const data = {
      ...form,
      price,
      salePrice: form.onSale ? salePrice : null,
  ...
  const getDiscountPercent = (p) => {
    if (!p.onSale || !p.salePrice || !p.price) return null;
        ...
        >
          ...
          <form onSubmit={save}>
            ...
            >
              ...
              <div>
                <label style={labelStyle}>السعر الأصلي (جنيه) *</label>
                <input
                  name="price"
                  type="number"
                  value={form.price}
              ...
              <div>
                <label style={labelStyle}>هل يوجد عرض؟</label>
                <select
                  value={form.onSale ? 'yes' : 'no'}
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      onSale: e.target.value === 'yes',
                    }))
                  }
                  style={inputStyle}
                >
              ...
              {form.onSale && (
                <div>
                  <label style={labelStyle}>سعر العرض (جنيه)</label>
                  <input
                    type="number"
                    ...
                    style={inputStyle}
                  />
                  <p style={hintStyle}>لازم يكون أقل من السعر الأصلي</p>
                </div>
              )}
                  ...
                  {currentColors.map((color) => (
                    ...
                    >
                      ...
                      <input
                        ...
                        accept="image/*"
                        onChange={(e) =>
                          e.target.files[0] && uploadColorImage(color, e.target.files[0])
                        }
              ...
              <div style={{ padding: 16 }}>
                ...
                {p.onSale && p.salePrice ? (
                  <div style={{ marginBottom: 12 }}>
                    ...
                    >
                      {p.salePrice?.toLocaleString()} جنيه
                    </div>
                  </div>
                ) : (
                  <p
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button
                    ...
                    style={{
                      flex: 1,
                      padding: '6px',
                      background: p.isActive ? '#fef3c7' : '#d1fae5',
                      border: 'none',
                      cursor: 'pointer',
                      borderRadius: 4,
                      fontSize: '0.8rem',
