...
const getShippingCost = (city, subtotal) => {
  if (!city) return 0;
  if ((subtotal || 0) >= FREE_SHIPPING_LIMIT) return 0;
  return NEAR_CAIRO.includes(city) ? 70 : 100;
};
...
export default function CheckoutPage() {
  ...
  const applyAddress = (address) => {
    setForm((prev) => ({
      ...
      city: address.city || '',
      address: `${address.area || ''} - ${address.street || ''}${address.details ? ` - ${address.details}` : ''}`,
    }));
  };
  ...
  const shippingCost = getShippingCost(form.city, total);
  const isFreeShipping = form.city && (total || 0) >= FREE_SHIPPING_LIMIT;
  ...
  const saveAddressIfNeeded = async () => {
    if (!useNewAddress || !saveNewAddress || !user) return;

    const label = newAddressLabel.trim() || 'عنوان جديد';
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'orders'), {
        ...
        city: form.city,
        notes: form.notes,
        selectedAddressId: useNewAddress ? null : selectedAddressId || null,
        items: items.map((i) => ({
          id: i.id,
          name: i.name,
          price: i.price,
      ...
      <>
        ...
        >
          ...
          <p style={{ color: 'var(--gray)', marginBottom: 24 }}>
            سنتواصل معك على <strong>{form.phone}</strong> لتأكيد التوصيل
          </p>
            ...
            <div>
                ...
                <div style={{ marginBottom: 26 }}>
                  ...
                  <div className="saved-addresses">
                    {addresses.map((a) => (
                      <label
                        key={a.id}
                        className={`address-card ${selectedAddressId === a.id && !useNewAddress ? 'active' : ''}`}
                      >
                        <input
                          type="radio"
                          name="selectedAddress"
                          checked={selectedAddressId === a.id && !useNewAddress}
                    ...
                    <label className={`address-card ${useNewAddress ? 'active' : ''}`}>
                      <input
                        type="radio"
                        name="selectedAddress"
                      ...
                      />
                      <div>
                        <strong>استخدام عنوان جديد</strong>
                        <p style={{ margin: '6px 0 0', fontSize: '0.85rem', color: '#777' }}>
                          اكتب بيانات توصيل مختلفة لهذا الطلب
                        </p>
                      </div>
                    </label>
                  </div>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                {form.city && (
                  ...
                  >
                    {isFreeShipping ? (
                      <>🎉 الشحن مجاني لأن طلبك عدى ١٠٠٠ جنيه</>
                    ) : (
                      <>
              ...
              <div style={{ marginBottom: 28, padding: 16, background: '#f9f7f4', borderRadius: 6, border: '1px solid var(--border)' }}>
                ...
                {appliedCoupon && (
                  <div style={{ marginTop: 8, color: '#166534', background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '7px 12px', borderRadius: 4, fontSize: '0.82rem' }}>
                    ✅ تم تطبيق <strong>{appliedCoupon.code}</strong> — خصم{' '}
                    {appliedCoupon.type === 'percent'
                ...
                {couponError && (
                  <div style={{ marginTop: 8, color: '#ef4444', fontSize: '0.82rem' }}>
                    ❌ {couponError}
                  </div>
                )}
              </div>

              <h3 style={{ margin: '8px 0 20px', fontWeight: 600 }}>
                طريقة الدفع
              </h3>
              ...
              <div style={{ background: '#f9f7f4', padding: 24, borderRadius: 8, position: 'sticky', top: 100 }}>
                ...
                <div style={{ borderTop: '1px solid var(--border)', marginTop: 16, paddingTop: 16 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800, fontSize: '1.1rem', borderTop: '1px solid var(--border)', paddingTop: 12, marginTop: 8 }}>
                    <span>الإجمالي</span>
                    <span>{!form.city ? '—' : `${grandTotal.toLocaleString()} جنيه`}</span>
                  </div>
                </div>
                ...
                >
                  {loading ? 'جاري التأكيد...' : `تأكيد الطلب${form.city ? ` — ${grandTotal.toLocaleString()} ج` : ''}`}
                </button>
